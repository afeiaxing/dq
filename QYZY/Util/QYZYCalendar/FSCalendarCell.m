...
- (void)drawView {
    ...
    self.currentSelectView = [[UILabel alloc] initWithFrame:CGRectMake(gap, gap/5, width, self.bounds.size.height - gap / 2.5)];
    self.currentSelectView.layer.cornerRadius = 4;
    self.currentSelectView.layer.masksToBounds = YES;
    [self addSubview:self.currentSelectView];
    
    self.solarDateLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, width, self.bounds.size.height - gap / 2.5)];
