...
/**
 ...
 The `cs_makeToast:` methods create a new view and then display it as toast.
 
 The `cs_showToast:` methods display any view as toast.
...
/**
 ...
 The look and feel is configured via the style. Unlike the `cs_makeToast:` methods,
 this method does not present the toast view automatically. One of the cs_showToast:
 ...
 */
- (UIView *)cs_toastViewForMessage:(NSString *)message
                             title:(NSString *)title
                             image:(UIImage *)image
                             style:(CSToastStyle *)style;
...
/**
 ...
 @see `cs_hideAllToasts` to remove all active toasts from a view.
 
 @warning This method has no effect on activity toasts. Use `cs_hideToastActivity` to
 ...
 */
- (void)cs_hideAllToasts:(BOOL)includeActivity
              clearQueue:(BOOL)clearQueue;

/**
 Removes all toast views from the queue. This has no effect on toast views that are
 active. Use `cs_hideAllToasts` to hide the active toasts views and clear the queue.
 */
...
/**
 ...
 calls to `cs_makeToastActivity:` will be ignored until cs_hideToastActivity is called.
 
 @warning `cs_makeToastActivity:` works independently of the cs_showToast: methods. Toast activity
 views can be presented and dismissed while toast views are being displayed. `cs_makeToastActivity:`
 has no effect on the queueing behavior of the cs_showToast: methods.
...
/**
 ...
 @param completion The completion block, executed after the toast view disappears.
                   didTap will be `YES` if the toast view was dismissed from a tap.
 @param enable The background view event is response.
 */
- (void)cs_showToast:(UIView *)toast
            duration:(NSTimeInterval)duration
            position:(id)position
          completion:(void(^)(BOOL didTap))completion
   backgroundEnabled:(BOOL)enable;

@end

/**
 `CSToastStyle` instances define the look and feel for toast views created via the
 `cs_makeToast:` methods as well for toast views created directly with
 `cs_toastViewForMessage:title:image:style:`.
 
 @warning `CSToastStyle` offers relatively simple styling options for the default
 toast view. If you require a toast view with more complex UI, it probably makes more
 sense to create your own custom UIView subclass and present it with the `cs_showToast:`
 methods.
 */
...
/**
 The size of the toast activity view when `cs_makeToastActivity:` is called.
...
/**
 ...
 a `cs_makeToast:` method (or `cs_toastViewForMessage:title:image:style:`) is called
...
/**
 Sets the default duration. Used for the `cs_makeToast:` and
 `cs_showToast:` methods that don't require an explicit duration.
 Default is 3.0.
...
/**
 Sets the default position. Used for the `cs_makeToast:` and
 `cs_showToast:` methods that don't require an explicit position.
