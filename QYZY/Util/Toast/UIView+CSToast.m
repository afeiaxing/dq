...
- (void)cs_makeToast:(NSString *)message {
    [self cs_makeToast:message duration:[CSToastManager defaultDuration] position:[CSToastManager defaultPosition] style:nil];
}

- (void)cs_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
    [self cs_makeToast:message duration:duration position:position style:nil];
}

- (void)cs_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position style:(CSToastStyle *)style {
    [self cs_makeToast:message duration:duration position:position title:nil image:nil style:style completion:nil];
}

- (void)cs_makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position title:(NSString *)title image:(UIImage *)image style:(CSToastStyle *)style completion:(void(^)(BOOL didTap))completion {
    UIView *toast = [self cs_toastViewForMessage:message title:title image:image style:style];
    ...
    [self cs_showToast:toast duration:duration position:position completion:completion backgroundEnabled:true];
}

#pragma mark - Show Toast Methods

- (void)cs_showToast:(UIView *)toast {
    [self cs_showToast:toast duration:[CSToastManager defaultDuration] position:[CSToastManager defaultPosition] completion:nil backgroundEnabled:true];
}

- (void)cs_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position completion:(void(^)(BOOL didTap))completion backgroundEnabled:(BOOL)enable {
    // sanity
    if (toast == nil) return;
...
- (void)cs_hideToast {
    [self cs_hideToast:[[self cs_activeToasts] firstObject]];
}
...
- (void)cs_hideAllToasts {
    [self cs_hideAllToasts:YES clearQueue:YES];
}

- (void)cs_hideAllToasts:(BOOL)includeActivity clearQueue:(BOOL)clearQueue {
    if (clearQueue) {
        [self cs_clearToastQueue];
    }
    ...
    if (includeActivity) {
        [self cs_hideToastActivity];
    }
}

- (void)cs_clearToastQueue {
...
- (void)cs_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position {
    ...
    [UIView animateWithDuration:[[CSToastManager sharedStyle] fadeDuration]
                     ...
                     animations:^{
                         ...
                         /// 修复获取当前topVC时未调用hide方法会导致内存泄漏
                         CSWeakProxy * weakProxy = [[CSWeakProxy alloc] initWithTarget:self];
                         NSTimer *timer = [NSTimer timerWithTimeInterval:duration target:weakProxy selector:@selector(cs_toastTimerDidFinish:) userInfo:toast repeats:NO];
...
- (UIView *)cs_toastViewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image style:(CSToastStyle *)style {
    // sanity
...
- (void)cs_makeToastActivity:(id)position {
    // sanity
